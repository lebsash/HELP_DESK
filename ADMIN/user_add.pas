...
if ((length(Edit1.Text)>0)and(length(Edit3.Text)>0)and(Length(MaskEdit1.Text)>0))or(Checkbox1.Visible)
then begin
...
if MaskEdit1.Text=MaskEdit2.Text then begin
 ...
 if Label11.Caption='1' then
   begin
     ...
     IBQuery4.SQL.Clear;
     IBQuery4.SQL.Append('select * from USERS WHERE FAMILY=:FAMILY AND NAME=:NAME AND ID_FIRM=:ID_FIRM');
     IBQuery4.ParamByName('FAMILY').AsString:=Edit3.Text;
     IBQuery4.ParamByName('NAME').AsString:=Edit1.Text;
     IBQuery4.ParamByName('ID_FIRM').AsInteger:=strtoint(label12.caption);
     IBQuery4.Close;
     IBQuery4.Open;
     IBQuery4.First;
     ...
     IBQuery1.ParamByName('ID_FIRM').AsInteger:=strtoint(label12.Caption);
  ...
  IBQuery6.ParamByName('ID').AsInteger:=num_ID;
   ...
   // Редактируем пользователя
    ...
    unit2.DataModule2.IBTransaction1.CommitRetaining;
     ...
     // Если пользователь доменный - сохраняем чеки
     if (checkbox1.Visible=true) and (checkbox2.Visible=true) then
        begin
           IBQuery5.SQL.Clear;
           IBQuery5.SQL.Append('UPDATE USERSGRAY SET DET=:DET WHERE NAME_LOGIN=:NAME_LOGIN');
           if (checkbox1.Checked)and(checkbox2.Checked) then  IBQuery5.ParamByName('DET').AsInteger:=1;
           if (checkbox1.Checked)and(checkbox2.Checked=false) then  IBQuery5.ParamByName('DET').AsInteger:=0;
           if (checkbox1.Checked=false) then  IBQuery5.ParamByName('DET').AsInteger:=9;
           IBQuery5.ParamByName('NAME_LOGIN').AsString:=Label15.Caption;
           IBQuery5.Close;
           IBQuery5.Open;
           unit2.DataModule2.IBTransaction1.CommitRetaining;
        end;
